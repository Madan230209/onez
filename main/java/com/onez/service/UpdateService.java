...
public class UpdateService {
	 ...
	 */
	public Boolean updateUserInfo(UserModel student) {
		if (isConnectionError) {
			return null;
		}

		int programId = getProgramId(student.getProduct().getName());
		if (programId == 0) {
			// Handle case where the program is not found in the database
			System.out.println("Invalid program: " + student.getProduct().getName());
